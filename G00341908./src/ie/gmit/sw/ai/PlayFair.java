...
public class PlayFair extends Menu {
	...
	public void PlayFairCipher() throws Exception {
		Scanner scan = new Scanner(System.in);
		System.out.println("Tonra's Playfair Cipher ");
		System.out
				.println("Would you like to (encrypt) or (decrypt) a message? (type one)");

		String input = scan.nextLine();
		if (input.equals("encrypt") || input.equals("e")
				|| input.equals("decrypt") || input.equals("d")) {
			if (input.equals("encrypt") || input.equals("e")) {
			...
			else if (input.equals("decrypt") || input.equals("d")) {
				...
				System.out
						.println("Decrypted message (ignore extra Xs and Qs):");
				for (Digraph d : ddigraphs) {
					System.out.print(d.c1);
					System.out.print(d.c2);
				}
	...
	public PlayFair() throws Exception {
		super();
	...
	private static void buildKeyTable() {
		...
		for (int i = 0; i < 5; i++) {
			for (int j = 0; j < 5; j++) {
				...
				rowval.put(keyword.charAt(k), i); // while we're here let's
													// store the row
				colval.put(keyword.charAt(k), j); // and col values of the chars
													// in a fast hashtable
				k++;
			}
		}
	...
	private static void buildDigraphList() {
		...
		ciphertext = ciphertext.replaceAll("[^a-zA-Z]", "");// remove all non
															// alphabet
															// characters
		...
		for (int i = 0; i < newtext.length(); i = i + 2) {
			Digraph toadd = new Digraph(newtext.charAt(i),
					newtext.charAt(i + 1));
			cdigraphs.add(toadd);
		}
	}
	...
	private static void encryptDigraphList() {
		...
		for (Digraph d : cdigraphs) {
			...
			// System.out.println(one+" row "+rowone+" col "
			// +colone+" "+two+" row "+rowtwo+" col "+coltwo);
			...
			if (rowone == rowtwo) {
				...
				System.out.println("Column replacing with: " + newd.c1
						+ newd.c2);
			}
				...
				System.out.println("Rectangle replacing with: " + newd.c1
						+ newd.c2);
			}
